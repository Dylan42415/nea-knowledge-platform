---
title: Fine Particulate Matter (PM2.5)
type: Pollutant
source_document: soe_report.pdf
source_location: pp. 17, 18-20, 'Impact of COVID-19 Circuit Breaker'
extraction_date: '2026-08-07'
tags: []
---

# Fine Particulate Matter (PM2.5)


## Summary
Fine Particulate Matter (PM2.5) consists of microscopic airborne particles emitted by vehicles, industry, and combustion sources. During Singapore's 2020 Circuit Breaker, outdoor PM2.5 levels dropped by 28.8% due to reduced road traffic, though overall 2020 annual levels still exceeded the WHO AQG (2005) limit.

## Key Data / Findings

| Metric / Parameter | Value | Benchmark / Baseline |
|---|---|---|
| Circuit Breaker Level Change | -28.8% | vs 2017–2019 baseline period |
| 2020 WHO AQG (2005) Status | Exceeded | Annual air quality standard |

## Relationships
- **COMPUTED_INTO** → [[Pollutant Standards Index (PSI)]]
- **BENCHMARKED_AGAINST** → [[WHO Air Quality Guidelines (2005)]]
- **MANAGED_BY** → [[National Environment Agency]]

## Source Excerpt
> Reductions in the atmospheric levels of outdoor air pollutants such as fine particulate matter (PM2.5) (-28.8%)... were observed during the CB period compared to the same time period in the previous three years
 — soe_report.pdf, pp. 17, 18-20, 'Impact of COVID-19 Circuit Breaker'
